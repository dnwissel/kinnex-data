...
from SQANTI3.utilities.short_reads import *
...
try:
    from SQANTI3.sequence.BED import LazyBEDPointReader
    from SQANTI3.sequence.err_correct_w_genome import err_correct
    from SQANTI3.sequence.sam_to_gff3 import convert_sam_to_gff3
    from SQANTI3.sequence.STAR import STARJunctionReader
...
if v1 < 8 or v2 < 6:
    print(
        "Cupcake version must be 8.6 or higher! Got {0} instead.".format(
            cupcake.__version__
        ),
        file=sys.stderr,
    )
...
if distutils.spawn.find_executable(GTF2GENEPRED_PROG) is None:
    print(
        "Cannot find executable {0}. Abort!".format(GTF2GENEPRED_PROG), file=sys.stderr
    )
    sys.exit(-1)
...
def correctionPlusORFpred(args, genome_dict):
    ...
    if os.path.exists(corrFASTA):
        print(
            "Error corrected FASTA {0} already exists. Using it...".format(corrFASTA),
            file=sys.stderr,
        )
    else:
        if args.fasta:
            if os.path.exists(corrSAM):
                print(
                    "Aligned SAM {0} already exists. Using it...".format(corrSAM),
                    file=sys.stderr,
                )
            else:
                ...
                if subprocess.check_call(cmd, shell=True) != 0:
                    print(
                        "ERROR running alignment cmd: {0}".format(cmd), file=sys.stderr
                    )
                    sys.exit(-1)
            ...
            with open(args.isoforms, "r") as isoforms_gtf:
                ...
                if ind == 0:
                    print(
                        "WARNING: GTF has {0} no annotation lines.".format(
                            args.isoforms
                        ),
                        file=sys.stderr,
                    )
    ...
    if args.skipORF:
        ...
        print(
            "ORF file {0} already exists. Using it....".format(corrORF), file=sys.stderr
        )
        ...
        with open(corrORF, "w") as f:
            for r in SeqIO.parse(open(gmst_pre + ".faa"), "fasta"):
                ...
                if pos != -1:
                    ...
                    orfDict[r.id] = myQueryProteins(
                        cds_start, cds_end, orf_length, newseq, proteinID=newid
                    )
                    f.write(">{0}\n{1}\n".format(newid, newseq))
                else:
                    new_rec = r
...
def expression_parser(expressionFile):
    ...
    for exp_file in exp_paths:
        ...
        if all(k in reader.fieldnames for k in EXP_KALLISTO_HEADERS):
            ...
            print(
                "Expected Kallisto or RSEM file format from {0}. Abort!".format(
                    expressionFile
                ),
                file=sys.stderr,
            )
...
def transcriptsKnownSpliceSites(
    ...
    def gene_overlap(ref1, ref2):
        ...
        s1, e1 = min(start_ends_by_gene[ref1]["begin"]), max(
            start_ends_by_gene[ref1]["end"]
        )
        s2, e2 = min(start_ends_by_gene[ref2]["begin"]), max(
            start_ends_by_gene[ref2]["end"]
        )
        if s1 <= s2:
            return e1 <= s2
        else:
...
def write_junctionInfo(
    ...
    for junction_index, (d, a) in enumerate(trec.junctions):
        ...
        if (trec.chrom in junctions_by_chr) and (trec.chrom not in missing_chr):
            ...
            min_diff_s = -find_closest_in_list(
                junctions_by_chr[trec.chrom]["donors"], d
            )
            # find the closest junction end site
            min_diff_e = find_closest_in_list(
                junctions_by_chr[trec.chrom]["acceptors"], a
            )
        ...
        sample_cov = defaultdict(
            lambda: (0, 0)
        )  # sample -> (unique, multi) count for this junction
        ...
        qj = {
            ...
            "diff_to_Ref_start_site": (
                min_diff_s if min_diff_s == min_diff_s else "NA"
            ),  # check if min_diff is actually nan
            "diff_to_Ref_end_site": (
                min_diff_e if min_diff_e == min_diff_e else "NA"
            ),  # check if min_diff is actually nan
            "bite_junction": (
                "TRUE"
                if (
            ...
            "sample_with_cov": (
                sum([cov_uniq > 0 for (cov_uniq, cov_multi) in sample_cov.values()])
                if covInf is not None
                else "NA"
            ),
            "total_coverage_unique": (
                sum([cov_uniq for (cov_uniq, cov_multi) in sample_cov.values()])
                if covInf is not None
                else "NA"
            ),
            "total_coverage_multi": (
                sum([cov_multi for (cov_uniq, cov_multi) in sample_cov.values()])
                if covInf is not None
                else "NA"
            ),
        }
...
def isoformClassification(
    ...
    if args.coverage is not None:
        ...
        if args.short_reads is not None:
            ...
            star_out, star_index = star(
                args.genome, args.short_reads, args.dir, args.cpus
            )
    ...
    if args.SR_bam is not None:
        ...
        if args.short_reads is not None:
            ...
            if star_out is None:
                print(
                    "Starting STAR mapping. We need this for calculating TSS ratio. It may take some time..."
                )
                star_out, star_index = star(
                    args.genome, args.short_reads, args.dir, args.cpus
                )
    ...
    if args.polyA_motif_list is not None:
        ...
        for line in open(args.polyA_motif_list):
            ...
            if any(s not in ("A", "T", "C", "G") for s in x):
                print(
                    "PolyA motif must be A/T/C/G only! Saw: {0}. Abort!".format(x),
                    file=sys.stderr,
                )
                sys.exit(-1)
    ...
    for chrom, records in isoforms_by_chr.items():
        for rec in records:
            ...
            if polyA_motif_list is not None:
                if rec.strand == "+":
                    polyA_motif, polyA_dist, polyA_motif_found = find_polyA_motif(
...
def FLcount_parser(fl_count_filename):
    ...
    if type == "SINGLE_SAMPLE":
        if "count_fl" not in count_header:
            print(
                "Expected `count_fl` field in count file {0}. Abort!".format(
                    fl_count_filename
                ),
                file=sys.stderr,
            )
        ...
        print(
            "Expected pbid or superPBID as a column in count file {0}. Abort!".format(
                fl_count_filename
            ),
            file=sys.stderr,
        )
...
def run(args):
    ...
    isoforms_info, ratio_TSS_dict = isoformClassification(
        args,
        isoforms_by_chr,
        refs_1exon_by_chr,
        refs_exons_by_chr,
    ...
    if args.fl_count:
        if not os.path.exists(args.fl_count):
            print(
                "FL count file {0} does not exist!".format(args.fl_count),
                file=sys.stderr,
            )
            sys.exit(-1)
        ...
        for pbid in fl_count_dict:
            if pbid not in isoforms_info:
                print(
                    "WARNING: {0} found in FL count file but not in input fasta.".format(
                        pbid
                    ),
                    file=sys.stderr,
                )
        if len(fl_samples) == 1:  # single sample from PacBio
            ...
            for iso in isoforms_info:
                if iso in fl_count_dict:
                    ...
                    print(
                        "WARNING: {0} not found in FL count file. Assign count as 0.".format(
                            iso
                        ),
                        file=sys.stderr,
                    )
    ...
    if ratio_TSS_dict is not None:
        ...
        for iso in ratio_TSS_dict:
            if iso not in isoforms_info:
                print(
                    "WARNING: {0} found in ratio TSS file but not in input FASTA/GTF".format(
                        iso
                    ),
                    file=sys.stderr,
                )
        for iso in isoforms_info:
            if iso in ratio_TSS_dict:
                ...
                print(
                    "WARNING: {0} not found in ratio TSS file. Assign count as 1.".format(
                        iso
                    ),
                    file=sys.stderr,
                )
    ...
    if args.expression:
        ...
        for iso in isoforms_info:
            if iso not in exp_dict:
                exp_dict[iso] = 0
                print(
                    "WARNING: isoform {0} not found in expression matrix. Assigning TPM of 0.".format(
                        iso
                    ),
                    file=sys.stderr,
                )
        ...
        if args.short_reads is not None:
            ...
            expression_files = kallisto(
                corrFASTA, args.short_reads, args.dir, args.cpus
            )
    ...
    #     cmd = RSCRIPTPATH + " {d}/{f} {c} {j} {p} {d} {a} {b}".format(
    #         d=utilitiesPath,
    #         f=RSCRIPT_REPORT,
    #         c=outputClassPath,
    #         j=outputJuncPath,
...
def run_isoAnnotLite(correctedGTF, outClassFile, outJuncFile, outName, gff3_opt):
    if gff3_opt:
        ...
        ISOANNOT_CMD = (
            "python3 "
            + ISOANNOT_PROG
            + " {g} {c} {j} -gff3 {t} -o {o} -novel -stdout {i}".format(
                g=correctedGTF,
                c=outClassFile,
                j=outJuncFile,
                t=gff3_opt,
        ...
        ISOANNOT_CMD = (
            "python3 "
            + ISOANNOT_PROG
            + " {g} {c} {j} -o {o} -novel".format(
                g=correctedGTF, c=outClassFile, j=outJuncFile, o=iso_out
            )
        )
    ...
    def find(self, chrom, strand, query, search_window=10000):
        ...
        for tss0, start0, end1 in self.cage_peaks[(chrom, strand)].find(
            ...
            if not within_peak == "TRUE":
                within_peak, dist_peak = w, (query - tss0) * (
                    -1 if strand == "-" else +1
                )
            else:
...
def split_input_run(args):
    ...
    if os.path.exists(SPLIT_ROOT_DIR):
        print(
            "WARNING: {0} directory already exists! Abort!".format(SPLIT_ROOT_DIR),
            file=sys.stderr,
        )
        sys.exit(-1)
    else:
...
def main():
    ...
    parser.add_argument(
        "isoforms",
        help="\tIsoforms (FASTA/FASTQ) or GTF format. It is recommended to provide them in GTF format, but if it is needed to map the sequences to the genome use a FASTA/FASTQ file with the --fasta option.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="\t\tInput are fusion isoforms, must supply GTF as input",
    )
    parser.add_argument(
        "--orf_input",
    ...
    parser.add_argument(
        "--fasta",
        help="\t\tUse when running SQANTI by using as input a FASTA/FASTQ with the sequences of isoforms",
        action="store_true",
    )
    parser.add_argument(
        "-e",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="\t\tInclude saturation curves into report",
    )
    parser.add_argument(
        "--report",
        choices=["html", "pdf", "both", "skip"],
        default="html",
        help="\t\tselect report format\t\t--html\t\t--pdf\t\t--both\t\t--skip",
    )
    parser.add_argument(
        "--isoAnnotLite",
    ...
    parser.add_argument(
        "--isoform_hits",
        help="\t\t Report all FSM/ISM isoform hits in a separate file",
        required=False,
        default=False,
        action="store_true",
    )
    ...
    if args.is_fusion:
        ...
        if args.fasta:
            print(
                "ERROR: if --is_fusion is on, must supply GTF as input", file=sys.stderr
            )
            sys.exit(-1)
    ...
    if args.gff3 is not None:
        ...
        if not os.path.isfile(args.gff3):
            print(
                "ERROR: Precomputed tappAS GFF3 annoation file {0} doesn't exist. Abort!".format(
                    args.genome
                ),
                file=sys.stderr,
            )
    ...
    if args.expression is not None:
        if os.path.isdir(args.expression) == True:
            print(
                "Expression files located in {0} folder".format(args.expression),
                file=sys.stderr,
            )
        else:
            for f in args.expression.split(","):
                if not os.path.exists(f):
                    print(
                        "Expression file {0} not found. Abort!".format(f),
                        file=sys.stderr,
                    )
                    sys.exit(-1)
    ...
    if args.dir is None:
        ...
        if os.path.isdir(args.dir):
            print(
                "WARNING: output directory {0} already exists. Overwriting!".format(
                    args.dir
                ),
                file=sys.stderr,
            )
    ...
    if not os.path.isfile(args.genome):
        print(
            "ERROR: genome fasta {0} doesn't exist. Abort!".format(args.genome),
            file=sys.stderr,
        )
        sys.exit()
    ...
    if not os.path.isfile(args.isoforms):
        print(
            "ERROR: Input isoforms {0} doesn't exist. Abort!".format(args.isoforms),
            file=sys.stderr,
        )
        sys.exit()
    ...
    if args.fasta:
        if args.aligner_choice == "gmap":
            if not os.path.isdir(os.path.abspath(args.gmap_index)):
                print(
                    "GMAP index {0} doesn't exist! Abort.".format(args.gmap_index),
                    file=sys.stderr,
                )
                sys.exit()
        elif args.aligner_choice == "deSALT":
            if not os.path.isdir(os.path.abspath(args.gmap_index)):
                print(
                    "deSALT index {0} doesn't exist! Abort.".format(args.gmap_index),
                    file=sys.stderr,
                )
                sys.exit()
    ...
    with open(args.doc, "w") as f:
        ...
        f.write(
            "FLCount\t"
            + (os.path.abspath(args.fl_count) if args.fl_count is not None else "NA")
            + "\n"
        )
        f.write(
            "Expression\t"
            + (
                os.path.abspath(args.expression)
                if args.expression is not None
                else "NA"
            )
            + "\n"
        ...
        f.write(
            "Junction\t"
            + (os.path.abspath(args.coverage) if args.coverage is not None else "NA")
            + "\n"
        )
        f.write(
            "CAGEPeak\t"
            + (os.path.abspath(args.CAGE_peak) if args.CAGE_peak is not None else "NA")
            + "\n"
        )
        f.write(
            "PolyAMotif\t"
            + (
                os.path.abspath(args.polyA_motif_list)
                if args.polyA_motif_list is not None
                else "NA"
            )
            + "\n"
        ...
        f.write(
            "PolyAPeak\t"
            + (
                os.path.abspath(args.polyA_peak)
                if args.polyA_peak is not None
                else "NA"
            )
            + "\n"
        ...
        f.write(
            "PhyloP\t"
            + (
                os.path.abspath(args.phyloP_bed)
                if args.phyloP_bed is not None
                else "NA"
            )
            + "\n"
        ...
        f.write(
            "ORFInput\t"
            + (os.path.abspath(args.orf_input) if args.orf_input is not None else "NA")
            + "\n"
        )
        ...
        f.write(
            "GMAPindex\t"
            + (
                os.path.abspath(args.gmap_index)
        ...
        f.write(
            "isoAnnotGFF3\t"
            + (os.path.abspath(args.gff3) if args.gff3 is not None else "NA")
            + "\n"
        )
        f.write(
            "ShortReads\t"
            + (
                os.path.abspath(args.short_reads)
                if args.short_reads is not None
                else "NA"
            )
            + "\n"
        ...
        f.write(
            "ShortReadsBAMs\t"
            + (os.path.abspath(args.SR_bam) if args.SR_bam is not None else "NA")
            + "\n"
        )
    ...
    if args.chunks == 1:
        ...
        if args.isoAnnotLite:
            ...
            outputClassPath, outputJuncPath = get_class_junc_filenames(args)
            run_isoAnnotLite(
                corrGTF, outputClassPath, outputJuncPath, args.output, args.gff3
            )
    else:
